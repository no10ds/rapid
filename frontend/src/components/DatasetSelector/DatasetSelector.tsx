...
const DatasetSelector = ({
  ...
  const handleLayerSelect = (value) => {
    if (value) {
      ...
      setLayer(null), setDataset(null), setDataset(null)
    }
  }
  ...
  const getUniqueDomains = () => [
    ...new Set(
      layerFilteredDatasetsList.map((dataset) => `${dataset.layer}/${dataset.domain}`)
    )
  ]
      ...
      <Row>
        ...
        <FormControl fullWidth sx={{ m: 1 }}>
          <Autocomplete
            ...
            groupBy={(dataset) =>
              `${(dataset as Dataset).layer}-${(dataset as Dataset).domain}`
            }
