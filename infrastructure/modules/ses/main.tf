locals {
  allowed_recipients_email_domains = concat(["simulator.amazonses.com"], split(",", var.allowed_recipients_email_domains))
  allowed_sender_email_addresses   = concat(["no-reply@${var.domain_name}"], var.allowed_sender_email_addresses)
}

resource "aws_ses_domain_identity" "ses_domain" {
  count  = var.ses_service ? 1 : 0
...
resource "aws_sns_topic_subscription" "email_subscription" {
  count     = var.ses_service ? length(var.ses_support_emails_for_cloudwatch_alerts) : 0
  topic_arn = aws_sns_topic.ses_notifications[0].arn
  protocol  = "email"
  endpoint  = var.ses_support_emails_for_cloudwatch_alerts[count.index]
}
...
data "aws_iam_policy_document" "ses_policy_document" {
  ...
  statement {
    ...
    condition {
      ...
      values   = local.allowed_sender_email_addresses
  ...
  statement {
    ...
    condition {
      ...
      values   = [for key in local.allowed_recipients_email_domains : "*@${key}"]
